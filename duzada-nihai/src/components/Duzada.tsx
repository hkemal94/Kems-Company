...
export default function Duzada({
  ...
  // Harita kipi: 'arazi' = MapLibre 3B harita, 'pin' = eski parşömen pin haritası
  const [haritaKipi, setHaritaKipi] = useState<'arazi' | 'pin'>('arazi');
          ...
          {haritaKipi === 'arazi' && (
            <div className="lg:col-span-2 bg-[#E7EBE6] border border-[#B9C7BD] rounded-xl p-4 archive-shadow relative paper-grain">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  type="button"
                  onClick={() => setHaritaKipi('pin')}
                  className="px-2.5 py-1 text-xs font-mono font-bold rounded-md border cursor-pointer transition-all bg-white hover:bg-stone-50 text-[#1B2A4A] border-stone-300"
                >
                  📍 Pin Haritası
                </button>
              </div>
